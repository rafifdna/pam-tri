...
function set_service() {
  ...
  confirm="n"
  read_from_input confirm "$(gettext 'Do you need to customize the PAM-TRI external port')?" "y/n" "${confirm}"
  if [[ "${confirm}" == "y" ]]; then
    read_from_input http_port "$(gettext 'PAM-TRI web port')" "" "${http_port}"
    set_config HTTP_PORT "${http_port}"

    if [[ "${use_xpack}" == "1" ]]; then
      read_from_input ssh_port "$(gettext 'PAM-TRI ssh port')" "" "${ssh_port}"
      set_config SSH_PORT "${ssh_port}"
      read_from_input rdp_port "$(gettext 'PAM-TRI rdp port')" "" "${rdp_port}"
